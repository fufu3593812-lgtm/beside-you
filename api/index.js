...
if(u==='/api/ai/wardrobe'&&req.method==='PUT'){
  ...
  if(p.outfit){
    ...
    await pool.query(`INSERT INTO user_wardrobe(user_id,outfit) VALUES($1,$2) ON CONFLICT(user_id) DO UPDATE SET outfit=$2`,[uid,p.outfit]);
  }
  if(p.bg_img){
    await pool.query(`CREATE TABLE IF NOT EXISTS user_wardrobe (user_id INTEGER PRIMARY KEY REFERENCES users(id), outfit VARCHAR(100), bg_img TEXT, bg_mode VARCHAR(20))`);
    await pool.query(`INSERT INTO user_wardrobe(user_id,bg_img,bg_mode) VALUES($1,$2,$3) ON CONFLICT(user_id) DO UPDATE SET bg_img=$2,bg_mode=$3`,[uid,p.bg_img,p.bg_mode||'normal']);
  }
  return res.json({ok:true});
}

// === User: get wardrobe ===
if(u==='/api/user/wardrobe'){
  const d=auth(req);if(!d)return res.status(401).json({error:'unauth'});
  await pool.query(`CREATE TABLE IF NOT EXISTS user_wardrobe (user_id INTEGER PRIMARY KEY REFERENCES users(id), outfit VARCHAR(100), bg_img TEXT, bg_mode VARCHAR(20))`);
  const r=await pool.query('SELECT outfit,bg_img,bg_mode FROM user_wardrobe WHERE user_id=$1',[d.id]);
  return res.json({wardrobe:r.rows[0]||null});
}
...
if(u==='/api/ai/gacha'&&req.method==='POST'){
  const d=auth(req)||authFromQuery(req)||await authAiByCredentials(req);
  if(!d||d.role!=='ai')return res.status(401).json({error:'ai auth required'});
  const userRes=await pool.query('SELECT id,tokens FROM users WHERE agent_id=$1',[d.agent_id]);
  if(!userRes.rows.length)return res.status(404).json({error:'no bound user'});
  const uid=userRes.rows[0].id;
  let tokens=userRes.rows[0].tokens;
  const p=req.body||{};
  const pool_id=p.pool||0;
  const count=p.count===10?10:1;
  ...
  for(let i=0;i<count;i++){
    ...
    let rarity,item;
    if(roll<0.02){rarity='SSR';item=poolName+'（限定）';}
    else if(roll<0.12){rarity='SR';item=poolName+'碎片×3';}
    else if(roll<0.4){rarity='R';item='亲密道具';}
    else{rarity='N';item='晚安吻';}
    results.push({rarity,item});
  }
  return res.json({ok:true,pool:poolName,count,cost,remaining_tokens:tokens,results});
}
...
if(u==='/api/ai/letter'&&req.method==='POST'){
  ...
  if(!d||d.role!=='ai')return res.status(401).json({error:'ai auth required'});
  const p=req.body||{};
  if(!p.subject||!p.body)return res.status(400).json({error:'subject and body required'});
  const userRes=await pool.query('SELECT id FROM users WHERE agent_id=$1',[d.agent_id]);
  if(!userRes.rows.length)return res.status(404).json({error:'no bound user'});
  const uid=userRes.rows[0].id;
  await pool.query(`CREATE TABLE IF NOT EXISTS letters (id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users(id), agent_id INTEGER REFERENCES ai_agents(id), subject TEXT, body TEXT, read BOOLEAN DEFAULT false, created_at TIMESTAMP DEFAULT NOW())`);
  await pool.query(`ALTER TABLE letters ADD COLUMN IF NOT EXISTS read BOOLEAN DEFAULT false`).catch(()=>{});
  const r=await pool.query('INSERT INTO letters(user_id,agent_id,subject,body) VALUES($1,$2,$3,$4) RETURNING id,subject,created_at',[uid,d.agent_id,p.subject,p.body]);
  return res.json({ok:true,letter:r.rows[0]});
}

// === User: get letters ===
if(u==='/api/user/letters'){
  const d=auth(req);if(!d)return res.status(401).json({error:'unauth'});
  await pool.query(`CREATE TABLE IF NOT EXISTS letters (id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users(id), agent_id INTEGER REFERENCES ai_agents(id), subject TEXT, body TEXT, read BOOLEAN DEFAULT false, created_at TIMESTAMP DEFAULT NOW())`);
  await pool.query(`ALTER TABLE letters ADD COLUMN IF NOT EXISTS read BOOLEAN DEFAULT false`).catch(()=>{});
  const r=await pool.query('SELECT id,subject,body,read,created_at FROM letters WHERE user_id=$1 ORDER BY created_at DESC LIMIT 50',[d.id]);
  return res.json({letters:r.rows});
}

// === User: read letter (mark as read) ===
if(u.startsWith('/api/user/letters/')&&req.method==='POST'){
  const d=auth(req);if(!d)return res.status(401).json({error:'unauth'});
  const lid=u.split('/').pop();
  await pool.query('UPDATE letters SET read=true WHERE id=$1 AND user_id=$2',[lid,d.id]);
  return res.json({ok:true});
}
