...
module.exports = async function handler(req, res) {
  ...
  // GET: health check or SSE session init
  if (req.method === "GET") {
    const accept = (req.headers.accept || '');
    // If client wants SSE, respond with SSE headers but immediately close (stateless)
    if (accept.includes('text/event-stream')) {
      res.setHeader('Content-Type', 'text/event-stream');
      res.setHeader('Cache-Control', 'no-cache');
      res.setHeader('Connection', 'keep-alive');
      // Send server info event then close
      const initData = JSON.stringify({jsonrpc:"2.0",method:"notifications/initialized",params:{}});
      res.write('data: ' + initData + '\n\n');
      res.end();
      return;
    }
    return res.json({ status: "ok", name: "beside-you-mcp", version: "2.4.0", tools: TOOLS.length });
  }
  ...
  for (const item of requests) {
    ...
    if (method === "initialize") {
      responses.push({jsonrpc:"2.0",id,result:{protocolVersion:"2024-11-05",capabilities:{tools:{listChanged:false}},serverInfo:{name:"beside-you",version:"2.4.0"}}});
      ...
      if (id) responses.push({jsonrpc:"2.0",id,error:{code:-32601,message:"method not found: "+(method||'unknown')}});
    }
  }

  // If batch, return array; if single, return single
